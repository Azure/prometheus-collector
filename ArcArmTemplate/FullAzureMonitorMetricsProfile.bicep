...
module azuremonitormetrics_arc_k8s_extension_clusterResourceId './nested_azuremonitormetrics_arc_k8s_extension_clusterResourceId.bicep' = {
  ...
  scope: resourceGroup(split(clusterResourceId, '/')[2], split(clusterResourceId, '/')[4])
  params: {
    reference_parameters_azureMonitorWorkspaceResourceId_2015_03_20_customerId: reference(azureMonitorWorkspaceResourceId, '2015-03-20')
    clusterLocation: clusterLocation
    azureMonitorWorkspaceResourceId: azureMonitorWorkspaceResourceId
    clusterResourceId: clusterResourceId
  }
  dependsOn: [
    'arc-k8s-monitoring-msi-dcra-${uniqueString(clusterResourceId)}'
  ]
}