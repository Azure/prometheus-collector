  ...
  properties: {
    ...
    rules: [
      ...
      }
      {
        ...
        expression: 'sum by (namespace, controller, container, cluster)(((container_memory_working_set_bytes{container!="", image!="", container!="POD"} / on(namespace,cluster,pod,container) group_left kube_pod_container_resource_limits{resource="memory", node!=""})) * on(namespace, pod,  cluster) group_left(controller) label_replace(kube_pod_owner{owner_name=~"(.*)"}, "controller", "$1", "owner_name", "(.*)")) > .95'
        for: 'PT10M'
        annotations: {
      ...
      }
      {
        ...
        expression: 'sum by (cluster,container,controller,namespace)(kube_pod_container_status_last_terminated_reason{reason="OOMKilled"} * on(cluster,namespace,pod) group_left(controller) label_replace(kube_pod_owner{owner_name=~"(.*)"}, "controller", "$1", "owner_name", "(.*)")) > 0'
        for: 'PT5M'
        annotations: {
      ...
      }
      {
        ...
        expression: 'sum by (namespace,cluster,container,controller)(sum by (namespace,cluster,container,pod)(kubelet_volume_stats_used_bytes{job="kubelet"}) / sum by (namespace,cluster,container,pod)(kubelet_volume_stats_capacity_bytes{job="kubelet"}) * on(namespace, pod, cluster) group_left(controller) label_replace(kube_pod_owner{owner_name=~"(.*)"}, "controller", "$1", "owner_name", "(.*)")) >.8'
        for: 'PT5M'
        annotations: {
      ...
      }
      {
        ...
        expression: 'sum by (namespace, controller, container, cluster)(increase(kube_pod_container_status_restarts_total{job="kube-state-metrics"}[1h])* on(namespace, pod, cluster) group_left(controller) label_replace(kube_pod_owner{owner_name=~"(.*)"}, "controller", "$1", "owner_name", "(.*)"))  > 0 '
        for: 'PT15M'
        annotations: {
      ...
      }
      {
        ...
        expression: '1 - avg by (namespace,cluster,job,nodename)(node_memory_MemAvailable_bytes{job="node"}* on(instance) group_left(nodename) (node_uname_info)) / avg by (namespace,cluster,job,nodename)(node_memory_MemTotal_bytes{job="node"}* on(instance) group_left(nodename) (node_uname_info))  > .8'
        for: 'PT5M'
        annotations: {
      ...
      }
      {
        ...
        expression: 'sum by (cluster, namespace, controller) (kube_pod_status_phase{phase="failed"} * on(namespace, pod, cluster) group_left(controller) label_replace(kube_pod_owner{owner_name=~"(.*)"}, "controller", "$1", "owner_name", "(.*)"))  > 0'
        for: 'PT5M'
        annotations: {
  ...
  properties: {
    ...
    rules: [
      ...
      }
      {
        ...
        expression: 'sum by (namespace, controller, cluster) (max by(namespace, pod, cluster) (kube_pod_status_phase{job="kube-state-metrics", phase=~"Pending|Unknown"}  ) * on(namespace, pod, cluster) group_left(controller)label_replace(kube_pod_owner ,"controller","$1","owner_name","(.*)"))'
        for: 'PT15M'
        labels: {
          severity: 'warning'
        }
