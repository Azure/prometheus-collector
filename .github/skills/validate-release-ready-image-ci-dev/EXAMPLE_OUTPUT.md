...
| 5c. Daemonset Config | ✅ | 8 scrape jobs: cadvisor, kubelet, node, node-configmap, kappie-basic, networkobservability-retina/hubble/cilium. 5 active targets, 0 down. All node-level jobs present and healthy. **Env var replacement verified** in `node-configmap` job: `$NODE_NAME` → `aks-armpool-55703419-vmss000000`, `$$NODE_NAME` → `aks-armpool-55703419-vmss000000`, `$NODE_IP` → `10.240.0.49`, `$$NODE_IP` → `10.240.0.49`, static_configs target `$NODE_IP:19100` → `10.240.0.49:19100`. Both single-dollar and double-dollar variants resolved correctly. Target labels confirmed: instance=`aks-armpool-55703419-vmss000000`, node_name_single_dollar_sign=`aks-armpool-55703419-vmss000000`, node_ip_single_dollar_sign=`10.240.0.49`, node_ip_double_dollar_sign=`10.240.0.49`. No raw `$NODE_NAME` or `$NODE_IP` strings remain in running config. |
