 ...
 */
function detectWindowsHostIP() {
    // Allow explicit override
    if (process.env.CDP_HOST)
        return process.env.CDP_HOST;
    try {
...
async function getOrOpenICMTab(incidentId) {
    ...
    throw new Error(`Could not find ICM tab for incident ${incidentId} after opening`);
}
/**
 ...
 */
async function captureIncidentData(session, incidentId) {
    ...
    if (!summary && !description && discussions.length === 0)
        return null;
...
function getWindowsUser() {
    if (process.env.WINDOWS_USER)
        return process.env.WINDOWS_USER;
    try {
        const out = execSync("powershell.exe -NoProfile -Command \"$env:USERNAME\" 2>/dev/null", { encoding: "utf-8", timeout: 5000 }).trim().replace(/\r/g, "");
        if (out)
            return out;
    }
    catch { }
 ...
 */
function hasPortProxy(proxyPort, cdpPort) {
    try {
        const out = execSync('powershell.exe -NoProfile -Command "netsh interface portproxy show v4tov4" 2>/dev/null', { encoding: "utf-8", timeout: 5000 });
 ...
 */
function ensurePortProxy(proxyPort, cdpPort) {
    if (hasPortProxy(proxyPort, cdpPort))
        return;
    try {
        execSync(`powershell.exe -NoProfile -Command "netsh interface portproxy add v4tov4 listenport=${proxyPort} listenaddress=0.0.0.0 connectport=${cdpPort} connectaddress=127.0.0.1" 2>/dev/null`, { encoding: "utf-8", timeout: 10000 });
    }
    catch {
 ...
 */
function launchEdge(cdpPort, winUser) {
    try {
        execSync(`powershell.exe -NoProfile -Command "Start-Process 'C:\\Program Files (x86)\\Microsoft\\Edge\\Application\\msedge.exe' -ArgumentList '--remote-debugging-port=${cdpPort}','--user-data-dir=C:\\Users\\${winUser}\\.edge-cdp','--no-first-run'" 2>/dev/null`, { encoding: "utf-8", timeout: 10000 });
    }
    catch {
 ...
 */
async function waitForCDP(endpoint, maxWaitSec) {
...
export async function scrapeICMIncident(incidentId) {
    const cdpPort = 9222;
    ...
    if (!cdpReady) {
        ...
        const setupLog = ["🔧 Edge CDP not running — attempting auto-setup..."];
    ...
    try {
        ...
        if (details?.summary) {
            ...
            const armIds = details.summary.match(/\/subscriptions\/[a-f0-9-]+\/resource[Gg]roups\/[^"'<>\s&;]+/gi) || [];
            if (armIds.length > 0) {
        ...
        if (pageText && pageText.length > 100) {
            ...
            lines.push("⚠️ Could not extract authored summary or page content. The page may still be loading — retry in a few seconds.");
        }
        return lines.join("\n");
    }
    catch (err) {
...
//# sourceMappingURL=icm-browser.js.map