...
async function callKqlm(monitoringAccount, metricNamespace, query, startTime, endTime) {
 ...
 */
function kqlmLastValue(kqlm, seriesIndex = 0) {
    if (!kqlm.Results || kqlm.Results.length <= seriesIndex)
        return null;
 ...
 */
function kqlmTotalValue(kqlm, seriesIndex = 0) {
    if (!kqlm.Results || kqlm.Results.length <= seriesIndex)
        return 0;
    const vals = kqlm.Results[seriesIndex].SamplingTypes?.Sum;
    if (!vals || vals.length === 0)
...
export async function queryMdmThrottling(monitoringAccount, timeRangeHours = 6) {
    ...
    if (eventUsage?.values && eventLimit?.values &&
    ...
    if (tsUsage?.values && tsLimit?.values &&
    ...
    try {
        ...
        if (nsResult && nsResult.Status === "Success" && nsResult.Results && nsResult.Results.length > 0) {
            ...
            if (nsSummaries.length > 0) {
                ...
                lines.push("", "### Time Series by Namespace", "", "| Namespace | Active Time Series |", "|-----------|-------------------|");
    ...
    const reasonPromises = dropMetricsWithReasons.map(async (dm) => {
        ...
        if (!metricResult || metricResult.status !== "warning")
            return null;
        try {
            ...
            const section = [
                ...
                "|--------|---------------|--------|",
                ...rows.map((r) => `| \`${r.reason}\` | ${r.total.toLocaleString()} | ${r.latest.toLocaleString()} |`),
            ];
            return section.join("\n");
        }
        catch {
    ...
    lines.push("");
    if (anyThrottled) {
        lines.push("### ⚠️ ACTION REQUIRED", "Throttling or drops detected. Check the [Geneva QoS Dashboard](https://portal.microsoftgeneva.com/dashboard/mac_91c1e6c2-bcdf-4650-9f80-179b245c2533/GenevaQos/%E2%86%90%20MdmQos) for details.", "", "**Common drop reasons:**", "- `Duplicated` — MStore rejected samples as duplicates (same metric+labels+timestamp). Often caused by multiple collectors scraping the same target or HPA rebalancing during scale events", "- `NotSupportedDoubleValue` — Metric value is NaN/Inf/subnormal, which MDM cannot store", "- `ThrottledByQuota` — Account quota exceeded", "- `DimensionLimitExceeded` — Too many dimensions on the metric (limit: 64 for Prometheus accounts)", "- `TimeSeriesLimitExceeded` — Too many unique time series in the account");
    }
    else {
 ...
 */
export async function queryMdmMetric(monitoringAccount, metric, cluster, nameSpace = "customdefault", dimensions = "{}", timeRangeHours = 24) {
    const serverUp = await checkMdmServerHealth();
    if (!serverUp) {
        return [
            "## MDM Metric Query",
    ...
    try {
        ...
        if (!result.Success || !result.Result || result.Result.length === 0) {
            lines.push("", "⚠️ **No data returned.** Possible causes:", "- The metric name doesn't exist in this MDM account", "- The cluster name doesn't match the `cluster` dimension in MDM", "- No data was ingested for this metric in the queried time range", "- The namespace is wrong (try `prometheus` instead of `customdefault`)", "- The dimension filter is too restrictive");
            if (result.Error) {
        ...
        if (!parsed) {
            ...
            lines.push("", "✅ **Data exists** — metric returned results but could not parse Sum/Count arrays.", "", "**Raw response (first 2000 chars):**", "```", rawText.substring(0, 2000), "```");
            return lines.join("\n");
        }
        ...
        if (validSums.length === 0) {
            lines.push("", `⚠️ **All ${totalBuckets} buckets are NaN** — no actual data points in this time range.`, "The metric may have existed historically but has no current data.");
            return lines.join("\n");
        }
        ...
        lines.push("", `✅ **Data exists** — ${activeBuckets} active buckets out of ${totalBuckets} total (${nanBuckets} NaN).`, "", "### Sum Series (primary metric values)", "", "| Stat | Value |", "|------|-------|", `| First value | ${sumFirst} |`, `| Latest value | ${sumLatest} |`, `| Min | ${sumMin} |`, `| Max | ${sumMax} |`, `| Avg | ${sumAvg.toFixed(2)} |`, `| Active buckets | ${activeBuckets} / ${totalBuckets} |`, `| NaN buckets | ${nanBuckets} |`, `| Bucket resolution | ~${bucketSizeMin} min |`);
        lines.push("", "### Count Series (number of data points per bucket)", "", "| Stat | Value |", "|------|-------|", `| Total data points | ${countTotal} |`, `| Avg per bucket | ${countAvg.toFixed(1)} |`, `| Latest bucket | ${countLatest} |`);
        ...
        if (significantChanges.length > 0) {
            lines.push("", `### Significant Changes (>${"20"}% bucket-to-bucket)`, "", `Found **${significantChanges.length}** bucket(s) with >20% change:`);
        ...
        if (sparkMax > sparkMin) {
            ...
            lines.push("", `### Recent Trend (last ${sparkBuckets} buckets)`, "", `\`${spark}\``, `Range: ${sparkMin.toFixed(1)} → ${sparkMax.toFixed(1)}`);
        }
        // Show first 500 chars of raw response for debugging
        lines.push("", "<details><summary>Raw MDM response (first 1000 chars)</summary>", "", "```", rawText.substring(0, 1000), "```", "", "</details>");
        return lines.join("\n");
    }
    catch (err) {
        lines.push("", `❌ **Query failed:** ${err instanceof Error ? err.message : String(err)}`);
        return lines.join("\n");
    }
}
const ME_QOS_METRICS = [
    {
 ...
 */
export async function queryMeInternalMetrics(monitoringAccount, timeRangeHours = 6) {
    const serverUp = await checkMdmServerHealth();
    if (!serverUp) {
        return [
    ...
    const aggregateResults = await Promise.all(ME_QOS_METRICS.map(async (def) => {
        try {
            const kqlmQuery = `metric("${def.metric}").samplingTypes("Sum")`;
            const kqlmResult = await callKqlm(monitoringAccount, "MetricsExtension2", kqlmQuery, startTime, endTime);
            if (!kqlmResult || kqlmResult.Status !== "Success" || !kqlmResult.Results || kqlmResult.Results.length === 0) {
                return { ...def, status: "no_data", summary: "No data", total: 0, max: 0, hasData: false };
            }
            const vals = kqlmResult.Results[0].SamplingTypes?.Sum;
            if (!vals || vals.length === 0) {
                return { ...def, status: "no_data", summary: "No data", total: 0, max: 0, hasData: false };
            ...
            const nonNan = vals.filter((v) => !isNaN(v));
            if (nonNan.length === 0) {
                return { ...def, status: "ok", summary: "No drops/errors ✅", total: 0, max: 0, hasData: false };
            }
            const total = nonNan.reduce((a, b) => a + b, 0);
            const max = Math.max(...nonNan);
            if (total === 0) {
                return { ...def, status: "ok", summary: `No drops/errors (${nonNan.length} data points, all zero) ✅`, total: 0, max: 0, hasData: true };
            }
            return {
                ...def,
                status: "warning",
            ...
            return { ...def, status: "error", summary: `Error: ${err instanceof Error ? err.message : String(err)}`, total: 0, max: 0, hasData: false };
        }
    }));
    // Summary table
    ...
    const healthResults = await Promise.all(ME_HEALTH_METRICS.map(async (def) => {
        try {
            const kqlmQuery = `metric("${def.metric}").samplingTypes("Sum")`;
            const kqlmResult = await callKqlm(monitoringAccount, "MetricsExtension2", kqlmQuery, startTime, endTime);
            if (!kqlmResult || kqlmResult.Status !== "Success" || !kqlmResult.Results || kqlmResult.Results.length === 0) {
            ...
            const nonNan = vals.filter((v) => !isNaN(v));
            if (nonNan.length === 0)
                return { panel: def.panel, metric: def.metric, value: "No data (NaN)" };
            const latest = nonNan[nonNan.length - 1];
    ...
    if (metricsWithDrops.length > 0) {
        ...
        lines.push("", "### ✅ No ME-level drops or errors detected.", "", "MetricsExtension is processing and publishing all metrics without drops.", "If metrics are still missing from MDM, the issue is likely at the **MDM backend/MStore level**.", "Check `tsg_mdm_throttling` for MDM-level throttling (especially `MStoreDroppedSamplesCount`).");
    }
    return lines.join("\n");
}
