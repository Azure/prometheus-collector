...
func TestMergeYAML_WithMultipleJobsEnabled_ThenMergedConfigIsComplete(t *testing.T) {
	...
	mergedConfig = deepMerge(mergedConfig, config1)
	mergedConfig = deepMerge(mergedConfig, config2)

	// Assert
	require.Equal(t, 1, len(mergedConfig), "Exactly one root element")
	require.NotNil(t, mergedConfig["scrape_configs"], "scrape_configs should not be nil")
	mergedConfigStr := fmt.Sprintf("%v", mergedConfig["scrape_configs"])
