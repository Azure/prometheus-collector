...
@networkobservabilityRegex_minimal = "networkobservability.*|hubble.*|cilium_drop.*|cilium_forward.*"
...
@networkobservabilityRegex_minimal_mac = "networkobservability.*|hubble.*|cilium_drop.*|cilium_forward.*"
...
def populateSettingValuesFromConfigMap(parsedConfig)
  begin
    ...
    networkobservabilityRegex = parsedConfig[:networkobservability]
    if !networkobservabilityRegex.nil? && networkobservabilityRegex.kind_of?(String)
      if !networkobservabilityRegex.empty?
        if isValidRegex(networkobservabilityRegex) == true
          @networkobservabilityRegex = networkobservabilityRegex
          ConfigParseErrorLogger.log(LOGGING_PREFIX, "Using configmap metrics keep list regex for networkobservability")
        else
          ConfigParseErrorLogger.logError(LOGGING_PREFIX, "Invalid keep list regex for networkobservability")
        end
      end
    else
      ConfigParseErrorLogger.logError(LOGGING_PREFIX, "networkobservabilityRegex either not specified or not of type string")
    end
  ...
  begin
    if @minimalIngestionProfile == "true"
      if @isMacMode == true
        ...
        @networkobservabilityRegex = @networkobservabilityRegex + "|" + @networkobservabilityRegex_minimal_mac
        ...
        @networkobservabilityRegex = @networkobservabilityRegex + "|" + @networkobservabilityRegex_minimal
...
regexHash["NETWORKOBSERVABILITY_METRICS_KEEP_LIST_REGEX"] = @networkobservabilityRegex
