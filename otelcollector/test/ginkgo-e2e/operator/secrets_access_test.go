...
// originalSettings stores the original configmap value for restoration.
 ...
 *   - The ama-metrics-settings-configmap should have secrets_access_namespaces configured
 *     to include the test namespace (or cluster is on K8s < 1.36 with cluster-wide secrets access)
 ...
 */
var _ = Describe("Secrets Access for Basic Auth in ServiceMonitors", Label(utils.OperatorLabel), Ordered, Serial, func() {
	...
	BeforeAll(func() {
		...
		By("Patching ama-metrics-settings-configmap to include the test namespace in secrets_access_namespaces")
		patchSecretsAccessNamespaces(secretsTestNamespace)
	})

	AfterAll(func() {
		By("Restoring the original configmap settings")
		restoreOriginalConfigmap()
	...
	// -------------------------------------------------------------------------
	// 8.1: ServiceMonitor with basicAuth in a configured namespace + RBAC present
	// -------------------------------------------------------------------------
	Context("when secrets_access_namespaces includes the test namespace and RBAC is present", func() {
		...
		AfterAll(func() {
			By("Cleaning up test resources for scenario 8.1")
	...
	// -------------------------------------------------------------------------
	// 8.2: ServiceMonitor with basicAuth in a namespace NOT in secrets_access_namespaces
	// -------------------------------------------------------------------------
	Context("when secrets_access_namespaces does NOT include the test namespace", func() {
		...
		AfterAll(func() {
			By("Cleaning up resources for scenario 8.2")
	...
	// -------------------------------------------------------------------------
	// 8.3: Configmap updated with new namespace, pod restarted
	// -------------------------------------------------------------------------
	Context("when configmap is updated to add a new namespace", func() {
		It("should verify the configmap setting is present", func() {
			configMap, err := K8sClient.CoreV1().ConfigMaps(amaMetricsNamespace).Get(
				context.Background(), "ama-metrics-settings-configmap", metav1.GetOptions{})
			if err != nil {
				Skip("ama-metrics-settings-configmap not found — skipping configmap update test")
			}
			...
			// Verify the secrets_access_namespaces field exists in the configmap
	...
	// -------------------------------------------------------------------------
	// 8.4: K8s < 1.36 with no secrets_access_namespaces — cluster-wide still works
	// -------------------------------------------------------------------------
	Context("when running on Kubernetes < 1.36", func() {
		It("should check the cluster version", func() {
	...
	// -------------------------------------------------------------------------
	// 8.5: K8s >= 1.36 with no secrets_access_namespaces — no cluster-wide access
	// -------------------------------------------------------------------------
	Context("when running on Kubernetes >= 1.36", func() {
...
// patchSecretsAccessNamespaces updates the ama-metrics-settings-configmap to include
// the given namespace in secrets_access_namespaces, saving the original value for restoration.
func patchSecretsAccessNamespaces(namespace string) {
	ctx := context.Background()
	cm, err := K8sClient.CoreV1().ConfigMaps(amaMetricsNamespace).Get(ctx, settingsConfigMap, metav1.GetOptions{})
	Expect(err).NotTo(HaveOccurred(), "Failed to get %s configmap", settingsConfigMap)

	settings, ok := cm.Data["prometheus-collector-settings"]
	Expect(ok).To(BeTrue(), "configmap missing prometheus-collector-settings key")

	// Save original for restoration
	originalSettings = settings

	// Check if secrets_access_namespaces already contains our namespace
	if strings.Contains(settings, namespace) {
		fmt.Printf("secrets_access_namespaces already contains %s\n", namespace)
		return
	}

	// Update the setting — find the existing line and append the namespace
	lines := strings.Split(settings, "\n")
	updated := false
	for i, line := range lines {
		trimmed := strings.TrimSpace(line)
		if strings.HasPrefix(trimmed, "secrets_access_namespaces") {
			// Parse the current value
			parts := strings.SplitN(trimmed, "=", 2)
			if len(parts) == 2 {
				currentVal := strings.Trim(strings.TrimSpace(parts[1]), "\"")
				if currentVal == "" {
					lines[i] = fmt.Sprintf("    secrets_access_namespaces = \"%s\"", namespace)
				} else {
					lines[i] = fmt.Sprintf("    secrets_access_namespaces = \"%s,%s\"", currentVal, namespace)
				}
				updated = true
			}
			break
	...
	if !updated {
		...
		lines = append(lines, fmt.Sprintf("    secrets_access_namespaces = \"%s\"", namespace))
	}

	cm.Data["prometheus-collector-settings"] = strings.Join(lines, "\n")
	_, err = K8sClient.CoreV1().ConfigMaps(amaMetricsNamespace).Update(ctx, cm, metav1.UpdateOptions{})
	Expect(err).NotTo(HaveOccurred(), "Failed to update configmap with secrets_access_namespaces")

	fmt.Printf("Updated %s to include %s in secrets_access_namespaces\n", settingsConfigMap, namespace)
}

// restoreOriginalConfigmap restores the original prometheus-collector-settings value.
func restoreOriginalConfigmap() {
	if originalSettings == "" {
		return
	}
	...
	cm, err := K8sClient.CoreV1().ConfigMaps(amaMetricsNamespace).Get(ctx, settingsConfigMap, metav1.GetOptions{})
	if err != nil {
		fmt.Printf("Warning: failed to get configmap for restoration: %v\n", err)
		return
	}

	cm.Data["prometheus-collector-settings"] = originalSettings
	_, err = K8sClient.CoreV1().ConfigMaps(amaMetricsNamespace).Update(ctx, cm, metav1.UpdateOptions{})
	if err != nil {
		fmt.Printf("Warning: failed to restore configmap: %v\n", err)
	}
}
