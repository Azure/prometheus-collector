...
The per-node alert uses explicit metric selectors. A metric-name regex selector returned HTTP 501 from the validation workspace, while the replacement query succeeded and identified the Windows 2025 node with the missing metrics. Its 30-minute pending period avoids alerting on brief scrape/recording-rule warmup; the separate target-health alert covers unavailable exporters.
