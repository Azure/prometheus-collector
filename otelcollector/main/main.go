...
import (
    ...
    cm "github.com/configmapparser/shared"
)
...
func main() {
    ...
    err := cm.MonitorInotify(outputFile)
    if err != nil {
        log.Fatal(err)
    }

    if ccpMetricsEnabled == "true" {
        cm.ConfigmapParserForCCP()
    } else {
    ...
    cm.StartMetricsExtensionWithConfigOverrides(meConfigFile)

    // Get ME version
    meVersion, err := cm.ReadVersionFile("/opt/metricsextversion.txt")
    if err != nil {
        fmt.Printf("Error reading ME version file: %v\n", err)
    } else {
        cm.FmtVar("ME_VERSION", meVersion)
    }

    // Get Golang version
    golangVersion, err := cm.ReadVersionFile("/opt/goversion.txt")
    if err != nil {
        fmt.Printf("Error reading Golang version file: %v\n", err)
    } else {
        cm.FmtVar("GOLANG_VERSION", golangVersion)
    }
    ...
    cm.StartCommand("/opt/microsoft/otelcollector/otelcollector", "--config", collectorConfig)
    ...
    if err != nil {
        ...
        cm.FmtVar("OTELCOLLECTOR_VERSION", string(otelCollectorVersion))
    }

    prometheusVersion, err := cm.ReadVersionFile("/opt/microsoft/otelcollector/PROMETHEUS_VERSION")
    if err != nil {
        fmt.Printf("Error reading Prometheus version file: %v\n", err)
    } else {
        cm.FmtVar("PROMETHEUS_VERSION", prometheusVersion)
    }
...
func healthHandler(w http.ResponseWriter, r *http.Request) {
    ...
    if !cm.IsProcessRunning("otelcollector") {
        status = http.StatusServiceUnavailable
        message = "OpenTelemetryCollector is not running."
    }

    if cm.HasConfigChanged("/opt/inotifyoutput.txt") {
        status = http.StatusServiceUnavailable
        message = "inotifyoutput.txt has been updated - config changed"
    }

    if cm.HasConfigChanged("/opt/inotifyoutput-mdsd-config.txt") {
        status = http.StatusServiceUnavailable
    ...
    if status != http.StatusOK {
        fmt.Printf(message)
        cm.WriteTerminationLog(message)
    }
}
