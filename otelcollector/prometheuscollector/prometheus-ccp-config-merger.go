...
func appendMetricRelabelConfig(yamlConfigFile, keepListRegex string) {
	...
	keepListMetricRelabelConfig := []map[string]interface{}{
		{"source_labels": []interface{}{"__name__"}, "action": "keep", "regex": keepListRegex},
	}

	scrapeConfigs, ok := config["scrape_configs"].([]interface{})
	if ok {
		for _, scfg := range scrapeConfigs {
			scfgMap, ok := scfg.(map[string]interface{})
			if ok {
				metricRelabelCfgs, ok := scfgMap["metric_relabel_configs"].([]interface{})
				if !ok {
					scfgMap["metric_relabel_configs"] = keepListMetricRelabelConfig
				} else {
		...
		err = ioutil.WriteFile(yamlConfigFile, cfgYamlWithMetricRelabelConfig, 0644)
		if err != nil {
