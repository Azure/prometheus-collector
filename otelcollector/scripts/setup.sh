...
if [ "${ARCH}" != "amd64" ]; then
  wget https://github.com/Azure/prometheus-collector/releases/download/v6.9.0-main-07-22-2024-2e3dfb56/metricsext2-2.2024.809.1746-1.cm2.aarch64.rpm
  sudo tdnf install -y metricsext2-2.2024.809.1746-1.cm2.aarch64.rpm
else
  wget https://github.com/Azure/prometheus-collector/releases/download/v6.9.0-main-07-22-2024-2e3dfb56/metricsext2-2.2024.809.1746-1.cm2.x86_64.rpm
  sudo tdnf install -y metricsext2-2.2024.809.1746-1.cm2.x86_64.rpm
fi
sudo tdnf list installed | grep metricsext2 | awk '{print $2}' > metricsextversion.txt
