...
if [ "${MAC}" == "true" ]; then
      ...
      inotifywait /etc/mdsd.d/config-cache/metricsextension/TokenConfig.json --daemon --recursive --outfile "/opt/inotifyoutput-mdsd-config.txt" --event modify,delete --format '%e : %T' --timefmt '+%s'
fi
