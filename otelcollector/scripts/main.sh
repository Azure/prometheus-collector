...
if [ "$AZMON_USE_DEFAULT_PROMETHEUS_CONFIG" = "true" ]; then
      ...
      /opt/microsoft/otelcollector27/otelcollector --config /opt/microsoft/otelcollector/collector-config-default.yml --log-level ERROR --log-format json --metrics-level none &> /opt/microsoft/otelcollector/collector-log.txt &
else
      echo "starting otelcollector...."
      /opt/microsoft/otelcollector27/otelcollector --config /opt/microsoft/otelcollector/collector-config.yml --log-level ERROR --log-format json --metrics-level none &> /opt/microsoft/otelcollector/collector-log.txt &
fi
