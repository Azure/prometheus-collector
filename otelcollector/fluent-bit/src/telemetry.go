	...
	for ; true; <-ksmTelemetryTicker.C {
		...
		for podId := 0; podId < len(p.Pods); podId++ {
			for containerId := 0; containerId < len(p.Pods[podId].Containers); containerId++ {
				Log(CommonProperties["osType"])
				Log(fmt.Sprintf("Container Name- %v\n", p.Pods[podId].Containers[containerId].Name))
				Log(fmt.Sprintf("cpuKsmUsageCoreNanoSeconds- %v\n", p.Pods[podId].Containers[containerId].Cpu.UsageCoreNanoSeconds))
				Log(fmt.Sprintf("memoryKsmWorkingSetBytes- %v\n", p.Pods[podId].Containers[containerId].Memory.WorkingSetBytes))
				...
				if strings.TrimSpace(p.Pods[podId].Containers[containerId].Name) != "ama-metrics-ksm" {
					message := fmt.Sprintf("Container name mismatch")
					Log(message)
					continue
				}
				if strings.TrimSpace(p.Pods[podId].Containers[containerId].Name) == "ama-metrics-ksm" {
